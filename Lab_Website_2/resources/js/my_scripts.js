...
//	var invis=document.getElementById("id").style.visibility
	if(toggle==0){
...
function loadStatsPage(){
	...
	for(row = 2; row < table.rows.length; row++){
		var current_row = table.rows[row];
		var home_score = parseInt(current_row.cells[2].innerHTML);
		var away_score = parseInt(current_row.cells[3].innerHTML);
		var away_team = current_row.cells[1].innerHTML;
		// console.log("row " + row + ", col 0 = " + current_row.cells[0].innerHTML);

		if(home_score > away_score){
			winner = "Unversity of Colorado";
			total_wins++;
		}
			...
			winner = "TIE";
			console.log("There was a tie in row " + row + "!");
		}

		// Set the winner column
		current_row.cells[4].innerHTML = winner;

	}
	document.getElementById("wins").innerHTML=total_wins;
	document.getElementById("losses").innerHTML=total_loses;
...
function loadPlayersPage(){
	...
	for(a; a<players.length; a++){
		var player=players[a].name;
		space=space+ "<a class= 'dropdown-item' href='#' onclick='switchPlayers("+a+")'>"+player+"</a>"
	}
	pMenu.innerHTML=space;
}
